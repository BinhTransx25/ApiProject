...
const getAllProducts = async (page, limit, keyword) => {
    try {
        ...
        let products = await ModelProduct
            .find(query, 'name price quantity categories description images shopOwner')
            .skip(skip)
            .limit(limit)
            .sort(sort);
        return products;
    ...
    try {
        ...
        const products = await ModelProduct
            .find(
                ...
                'name price quantity categories description images shopOwner')
            .skip(skip)
            .limit(limit)
            .sort(sort)
            .exec();
...
// Thêm sản phẩm
const insert = async (name, price, quantity, images, description, category_ids, shopOwner_id) => {
    try {
        let categories = [];
...
const update = async (id, name, price, quantity, images, description, category_ids, shopOwner_id) => {
    try {
